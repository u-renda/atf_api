...
class Product_category extends REST_Controller {
    ...
    function __construct()
    {
		...
		$this->load->model('product_category_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_product_category' => $id_product_category));
			
			if ($query->num_rows() > 0)
			{
                $delete = $this->the_model->delete($id_product_category);
			...
			{
				$data['id_product_category'] = 'not found';
				$validation = "error";
				$code = 400;
			}
		}
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_product_category_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
	...
	function update_post()
	{
		...
		if ($validation == 'ok')
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_product_category, $param);
