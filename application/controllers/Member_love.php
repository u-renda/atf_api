...
class Member_love extends REST_Controller {

    function __construct()
    {
        parent::__construct();
		$this->load->helper('fungsi');
		$this->load->model('member_love_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        $id_member_love = filter($this->post('id_member_love'));
        
		$data = array();
        if ($id_member_love == FALSE)
		{
			$data['id_member_love'] = 'required';
			$validation = "error";
			$code = 400;
		}
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_member_love' => $id_member_love));
			
			if ($query->num_rows() > 0)
			{
                $delete = $this->the_model->delete($id_member_love);
	...
	function info_get()
	{
		...
		$id_member_love = filter($this->get('id_member_love'));
		...
		if ($validation == 'ok')
		{
			...
			if ($id_member_love != '')
			{
				$param['id_member_love'] = $id_member_love;
			}
			...
			if ($query->num_rows() > 0)
			{
				...
				$data = array(
					'id_member_love' => $row->id_member_love,
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_member_love_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
	...
	function update_post()
	{
		...
		if ($validation == 'ok')
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_member_love, $param);
