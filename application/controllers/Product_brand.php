...
class Product_brand extends REST_Controller {
    ...
    function __construct()
    {
		...
		$this->load->model('product_brand_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_product_brand' => $id_product_brand));
			
			if ($query->num_rows() > 0)
			{
                $delete = $this->the_model->delete($id_product_brand);
			...
			{
				$data['id_product_brand'] = 'not found';
				$validation = "error";
				$code = 400;
			}
		}
	...
	function info_get()
	{
		...
		$id_product_brand = filter($this->get('id_product_brand'));
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_product_brand_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
	...
	function update_post()
	{
		...
		if ($validation == 'ok')
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_product_brand, $param);
