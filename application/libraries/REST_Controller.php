...
abstract class REST_Controller extends CI_Controller
{
	...
	protected $methods = array(
        'lists_get' => array('level' => 10, 'limit' => 150),
    );
	...
	public function _remap($object_called, $arguments)
	{
		...
		$use_key = ! (isset($this->methods[$controller_method]['key']) AND $this->methods[$controller_method]['key'] == FALSE AND $this->methods[$controller_method]['key'] != '');
		
		// Get that useless shitty key out of here
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			if (config_item('rest_enable_logging') AND $log_method AND !empty($this->rest->key))
			{
				$this->_log_request();
			}
		...
		{
			// Check the limit
			if (config_item('rest_enable_limits') AND !$this->_check_limit($controller_method, $this->rest->ignore_limits, $this->rest->key))
			{
			...
			{
				$this->_log_request($authorized, $this->rest->key);
			}
	...
	{
        /* connect to database log */
        $this->rest = $this->load->database('default', TRUE);
		...
		{
			if ( ! ($row = $this->rest->where(config_item('rest_key_column'), $key)->where('status', '1')->get(config_item('rest_keys_table'))->row()))
			{
				return FALSE;
			}
	 ...
	 * @return object
	 */
	protected function _log_request($authorized = FALSE, $key)
	{
                $this->rest->db = $this->load->database('log', TRUE);
                
		return $this->rest->db->insert(config_item('rest_logs_table'), array(
					'uri' => $this->uri->uri_string(),
					'method' => $this->request->method,
					'params' => $this->_args ? serialize($this->_args) : '',
					'api_key' => isset($key) ? $key : '',
	 ...
	 * @return boolean
	 */
	protected function _check_limit($controller_method, $ignore_limits, $key)
	{
                $this->rest->db = $this->load->database('log', TRUE);
                
		// They are special, or it might not even have a limit
		if ( empty($ignore_limits) OR !isset($this->methods[$controller_method]['limit']))
		{
